...
enum PlantBalance {
    ...
    /// 첫 설치에 성장으로 인정해 주는 오늘치 상한.
    ///
    /// 0 을 보여주면 앱이 고장난 것처럼 보인다 — 그래서 오늘 사용량은 인정한다.
    /// 다만 전체 로그를 소급하면 한 사이클치(즉시 이식)가 되므로 상한을 둔다.
    ///
    /// 목표와 **같은 비율**이어야 한다. 30,000 으로 고정했을 때는 가볍게 쓰는 사람의
    /// 첫날이 사이클의 1.5배라 설치하자마자 이식 버튼이 떠 있었다. 사이클의 2/28,
    /// 즉 이틀치로 맞춘다 — 기준 사용자에게는 예전과 같은 30,000 근처다.
    static let firstDayCreditDays = 2
    static func firstDayCredit(cycle: Int) -> Int {
        max(1, cycle * firstDayCreditDays / targetCycleDays)
    }

    /// 첫 설치에 지갑으로 인정해 주는 오늘치 원시 토큰 상한.
    /// 성장 상한과 같은 이틀치로 맞춘다 — 한쪽만 후하면 앞뒤가 안 맞는다.
    /// 아직 잰 값이 없는 시점이라 기준 하루치를 쓴다.
    static func firstDayRawCap(dailyRaw: Int) -> Int {
        max(1, dailyRaw * firstDayCreditDays)
    }
